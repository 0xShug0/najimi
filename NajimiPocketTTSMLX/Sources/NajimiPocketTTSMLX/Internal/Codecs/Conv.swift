...
public final class ConvTranspose1d: Module {
    ...
    private var cachedExpandedWeight: MLXArray?
    private var cachedExpandedGroups: Int?
    ...
    private func expandedWeightAndGroups() -> (MLXArray, Int) {
        if let cachedExpandedWeight, let cachedExpandedGroups {
            return (cachedExpandedWeight, cachedExpandedGroups)
        }
        ...
        if groups == inChannels && groups == outChannels {
            ...
            let expanded = wRep * eyeW
            cachedExpandedWeight = expanded
            cachedExpandedGroups = 1
            return (expanded, 1)
        } else if groups > 1 {
            fatalError("groups > 1 (non-depthwise) not supported in ConvTranspose1d")
        } else {
            cachedExpandedWeight = weight
            cachedExpandedGroups = groups
