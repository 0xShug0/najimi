...
public final class PocketRotaryEmbedding: Module {
    fileprivate final class Cache {
        ...
        var cachedSingleOffset: Int?
        var cachedSingleCos: MLXArray?
        var cachedSingleSin: MLXArray?

        init(invFreq: MLXArray) {
    ...
    public init(maxPeriod: Double = 10000.0, headDim: Int) {
        precondition(headDim % 2 == 0, "RoPE requires an even head dimension")
        self.maxPeriod = maxPeriod
        let half = headDim / 2
        let indices = MLXArray(stride(from: 0, to: half, by: 1)).asType(.float32)
        let freqScale = -Float(log(maxPeriod)) * 2.0 / Float(headDim)
        self.cache = Cache(invFreq: MLX.exp(indices * MLXArray(freqScale)).reshaped([1, 1, 1, half]))
        super.init()
    }

    public func callAsFunction(_ q: MLXArray, _ k: MLXArray, offset: Int) -> (MLXArray, MLXArray) {
        return pocketApplyRoPE(q: q, k: k, offset: offset, cache: cache)
    }
}

@inline(__always)
private func pocketApplyRoPE(
    q: MLXArray,
    k: MLXArray,
    offset: Int,
    cache: PocketRotaryEmbedding.Cache
) -> (MLXArray, MLXArray) {
    let b = q.shape[0]
    let t = q.shape[1]
    let h = q.shape[2]
    ...
    let roti: MLXArray
    if t == 1 {
        if cache.cachedSingleOffset != offset || cache.cachedSingleCos == nil || cache.cachedSingleSin == nil {
            let ts = MLXArray(Float(offset)).reshaped([1, 1, 1, 1])
            let angles = cache.invFreq * ts
            cache.cachedSingleCos = MLX.cos(angles)
            cache.cachedSingleSin = MLX.sin(angles)
            cache.cachedSingleOffset = offset
        }
        rotr = cache.cachedSingleCos!
        roti = cache.cachedSingleSin!
    } else {
...
public final class PocketStreamingMultiheadAttention: Module {
    ...
    public func callAsFunction(_ query: MLXArray, cache: KVCacheSimple?) -> MLXArray {
        ...
        let mask: MLXArray? = if t == 1 {
            nil
        } else {
            pocketCreateAdditiveCausalMask(t, offset: offset).asType(query.dtype)
        }
...
public final class PocketStreamingTransformer: Module {
    ...
    public init(dModel: Int, numHeads: Int, numLayers: Int, dimFeedforward: Int, maxPeriod: Double, layerScale: Float? = nil) {
        ...
        let rope = PocketRotaryEmbedding(maxPeriod: maxPeriod, headDim: dModel / numHeads)
        self.rope = rope
